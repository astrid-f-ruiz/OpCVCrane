...
def derive_raw_csv_name_from_trial_tag(trial_tag):
    """
    Convert trial tag into the standardized RAW csv stem.

    Examples:
      H_OL_21IN_T1  -> H_OL_21IN_T_RAW1.csv
      H_CL_12IN_T3  -> H_CL_12IN_T_RAW3.csv
      M_OL_38LB_T6  -> M_OL_38LB_T_RAW6.csv
    """
    tag = trial_tag.strip().strip('"').strip()

    if not tag:
        return None

    # Remove accidental extension if user typed one
    if tag.lower().endswith(".csv"):
        tag = tag[:-4]

    parts = tag.split("_")

    # Expected final part like T1, T3, T6
    last = parts[-1] if parts else ""

    if len(last) >= 2 and last[0].upper() == "T" and last[1:].isdigit():
        trial_num = last[1:]
        base_parts = parts[:-1]
        raw_stem = "_".join(base_parts + ["T", f"RAW{trial_num}"])
    else:
        # Fallback: just append _RAW if it does not match the trial pattern
        raw_stem = tag + "_RAW"

    return raw_stem + ".csv"


def default_raw_csv_path_from_config(config_path, trial_tag):
    """
    Save RAW csv in the same folder as the config file.
    """
    config_dir = os.path.dirname(os.path.abspath(config_path)) or os.getcwd()
    raw_name = derive_raw_csv_name_from_trial_tag(trial_tag)
    if raw_name is None:
        raw_name = "raw_tracking.csv"
...
def main():
    ...
    trial_tag = input(
        "Trial tag for output name, e.g. H_OL_21IN_T1, H_CL_12IN_T3, M_OL_38LB_T6: "
    ).strip().strip('"')
    
    default_output_csv = default_raw_csv_path_from_config(config_path, trial_tag)
    
    output_csv = input(
        f"Output RAW CSV path [default: {default_output_csv}]: "
    ).strip().strip('"')
    
    if not output_csv:
        output_csv = default_output_csv
    else:
        # If user only types a filename, save it beside the config file.
