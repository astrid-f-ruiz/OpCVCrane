...
def build_hsv_range_from_patch(hsv_patch, hue_pad=9, sat_pad=45, val_pad=42):
...
def contour_shape_diagnostics(contour):
    """
    Extra diagnostics to tell whether the marker contour is clean,
    bloated, partial, or asymmetric.
    """
    if contour is None or len(contour) < 3:
        return None
    ...
    return {
        ...
        "minrect_w_px": float(rect_w),
        "minrect_h_px": float(rect_h),
        "minrect_short_px": float(min(rect_w, rect_h)),
        "minrect_long_px": float(max(rect_w, rect_h)),
        "minrect_angle_deg": float(rect_angle),
...
def mask_chord_diameters(mask, center_xy):
    """
    Estimate marker diameter by measuring the connected mask width
    through the detected center horizontally and vertically.

    This helps diagnose whether equivalent-area diameter is being
    inflated by uneven halo/edge pixels.
    """
    if mask is None or center_xy is None:
        return None
    ...
    # horizontal chord through center
    x_left = cx
    while x_left - 1 >= 0 and mask[cy, x_left - 1] > 0:
        x_left -= 1

    x_right = cx
    while x_right + 1 < w and mask[cy, x_right + 1] > 0:
        x_right += 1

    chord_x = x_right - x_left + 1

    # vertical chord through center
    y_top = cy
    while y_top - 1 >= 0 and mask[y_top - 1, cx] > 0:
        y_top -= 1

    y_bot = cy
    while y_bot + 1 < h and mask[y_bot + 1, cx] > 0:
        y_bot += 1

    chord_y = y_bot - y_top + 1

    return {
        "chord_x_px": float(chord_x),
        "chord_y_px": float(chord_y),
        "chord_avg_px": float(0.5 * (chord_x + chord_y)),
...
def detect_marker_on_roi_frame(roi_frame_bgr, hsv_cfg, tracking_defaults):
    ...
    if metrics is not None:
        shape_metrics = contour_shape_diagnostics(contour)
        chord_metrics = mask_chord_diameters(mask, ctr)
    
        if shape_metrics is not None:
            metrics.update(shape_metrics)
    
        if chord_metrics is not None:
            metrics.update(chord_metrics)
...
def collect_marker_diameter_stats(video_path, roi_xywh, frame_indices, hsv_cfg, tracking_defaults):
    ...
    for idx in frame_indices:
        ...
        per_frame.append({
            ...
            "bbox_w_px": float(metrics.get("bbox_w_px", np.nan)),
            "bbox_h_px": float(metrics.get("bbox_h_px", np.nan)),
            "minrect_short_px": float(metrics.get("minrect_short_px", np.nan)),
            "minrect_long_px": float(metrics.get("minrect_long_px", np.nan)),
            "chord_x_px": float(metrics.get("chord_x_px", np.nan)),
            "chord_y_px": float(metrics.get("chord_y_px", np.nan)),
            "chord_avg_px": float(metrics.get("chord_avg_px", np.nan)),
            "circularity": float(metrics.get("circularity", np.nan)),
            "solidity": float(metrics.get("solidity", np.nan)),
            "bbox_aspect_ratio": float(metrics.get("bbox_aspect_ratio", np.nan))
        })
...
def combine_hsv_configs(hsv_cfgs):
    """
    Combine multiple auto-built HSV configs into one config by unioning ranges.
    Useful when the same marker changes hue/brightness across the video.
    """
    ranges = []
    medians = []
    ...
    for cfg in hsv_cfgs:
        if cfg is None:
            continue
        ranges.extend(cfg.get("ranges", []))
        if "median_hsv" in cfg:
            medians.append(cfg["median_hsv"])

    if medians:
        med_arr = np.array(medians, dtype=float)
        median_hsv = [int(round(v)) for v in np.median(med_arr, axis=0)]
    else:
        median_hsv = [0, 0, 0]
...
def get_strategic_frame_windows(frame_count, centers, window_size=30):
    """
    Build small frame windows around hand-selected representative frame centers.

    Example:
      center = 600, window_size = 30
      gives frames 585 to 614.

    This is better than start/middle/end when marker color changes
    across the video.
    """
    frame_count = int(frame_count)
    ...
    clean_centers = sorted(set(clean_centers))

    windows = {}

    for c in clean_centers:
        # Keep full window length when possible
        start = c - half
        start = max(0, start)
        start = min(start, max(0, frame_count - window_size))

        end = min(frame_count, start + window_size)

        label = f"f{c:05d}"
        windows[label] = list(range(start, end))
...
def main():
    ...
    frame_windows = get_strategic_frame_windows(
        frame_count=frame_count,
        centers=strategic_centers,
        window_size=30
    )
    
    representative_frames = {
        label: idxs[len(idxs) // 2]
        for label, idxs in frame_windows.items()
    }
    
    print("\nStrategic representative frame windows:")
    for label, idxs in frame_windows.items():
        print(f"  {label:<8}: frames {idxs[0]} to {idxs[-1]} | representative {representative_frames[label]}")
  ...
  #####################################################################
    ...
    # First-frame sample from your original click
    green_hsv_samples.append(
        build_hsv_range_from_patch(
            green_patch,
            hue_pad=10,
            sat_pad=41,
            val_pad=46
    ...
    # Add representative middle/end samples so hue shifts are captured
    for label, frame_idx_rep in representative_frames.items():
        # skip first frame if you already sampled it from the initial click
        if frame_idx_rep == 0:
            continue
        ...
        if rep_frame is None:
            print(f"  WARNING: Could not read {label} frame for green HSV sample.")
            continue
    
        rep_hsv = cv2.cvtColor(rep_frame, cv2.COLOR_BGR2HSV)
    
        green_click_rep = click_point_on_image(
            f"Click GREEN marker color - {label}",
            rep_frame,
            f"Click GREEN marker color on {label} frame {frame_idx_rep}",
            prompt_color=(0, 255, 0)
        )
    
        if green_click_rep is None:
            print(f"  Skipped green HSV sample for {label}.")
            continue
    
        patch_rep, _ = sample_patch(rep_hsv, green_click_rep, half_size=6)
        ...
        # Let user click the green marker on this representative frame.
        # This is still automatic range-building; you are only sampling the marker color.
        green_click_rep = click_point_on_image(
            f"Click GREEN marker color - {label}",
            rep_frame,
            f"Click GREEN marker color on {label} frame",
            prompt_color=(0, 255, 0)
        )
    ...
    green_hsv = combine_hsv_configs(green_hsv_samples)
    
    print("\nGREEN multi-sample HSV medians:")
    for k, med in enumerate(green_hsv.get("sample_medians_hsv", []), start=1):
        print(f"  sample {k}: {med}")
    ...
    if pivot_choice == "2":
        ...
        print("  2) Choose labels manually")
        selection_choice = input("Choose 1 or 2 [default 1]: ").strip()
        
        if selection_choice == "2":
            manual_txt = input("Enter labels comma-separated, e.g. f00200,f00600,f01200: ").strip()
    ...
    print("  1) Use 1-inch marker diameter(s) with strategic-frame averaging")
    scale_choice = "1"

    length_info = {}

    if scale_choice in ("1"):
        green_window_stats = {}
        pink_window_stats = {}
        ...
        for label, idxs in frame_windows.items():
            ...
            PRINT_GREEN_FRAME_DIAGNOSTICS = True
            if PRINT_GREEN_FRAME_DIAGNOSTICS:
                print(f"\nGREEN {label} frame diagnostics:")
                for row in gstats["per_frame"]:
                    if not row["valid"]:
                        continue
                    print(
                        f"  frame {row['frame_idx']:>6}: "
                        f"eq={row['equivalent_diameter_px']:.2f}, "
                        f"circ={row['enclosing_circle_diameter_px']:.2f}, "
                        f"bbox=({row['bbox_w_px']:.1f},{row['bbox_h_px']:.1f}), "
                        f"chord=({row['chord_x_px']:.1f},{row['chord_y_px']:.1f}), "
                        f"circly={row['circularity']:.3f}, "
                        f"solid={row['solidity']:.3f}"
                    )
        ...
        for label in frame_windows.keys():
        ...
        if run_sanity_check == "y":
            ...
            if measured_travel_in_txt:
                ...
                window_labels = list(frame_windows.keys())
                first_label = window_labels[0]
                last_label = window_labels[-1]
                
                start_x_stats = collect_marker_x_stats(
                    video_path, roi_xywh, frame_windows[first_label], green_hsv, tracking_defaults
                )
                end_x_stats = collect_marker_x_stats(
                    video_path, roi_xywh, frame_windows[last_label], green_hsv, tracking_defaults
                )
                ...
                if start_mean_x is None or end_mean_x is None:
                    ...
                    print(f"  First window {first_label} mean green x [px] : {start_mean_x:.3f}")
                    print(f"  Last  window {last_label} mean green x [px] : {end_mean_x:.3f}")
