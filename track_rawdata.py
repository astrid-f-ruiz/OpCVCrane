...
def main():
    ...
    min_area = int(tracking_defaults.get("min_contour_area_px", 30))
    ...
    output_csv = input('Output CSV filename [default: raw_tracking.csv]: ').strip()
    if not output_csv:
        output_csv = "raw_tracking.csv"
    ...
    while True:
        ...
        if show_preview:
            ...
            # Draw line between markers if both found
            if pivot_found and bob_found:
                ...
                # Optional raw angle preview only
            ...
            cv2.imshow("Dual Marker Tracking", display)
