...
def compute_placement_metrics(time_s, measured_x_in, ideal_x_in, move_end_s, goal_in=0.125, poststop_buffer_s=0.0):
    ...
    return delta, {
        ...
        "placement_goal_in": float(goal_in),
        "placement_pass": int(maxabs <= float(goal_in)),
...
def main():
    ...
    stationary_profile_path_used = r"D:/Documents/F25_real/OpenCV Test/Experimental/stationary_noise_profile.json"
    ...
    free_decay_start_s = move_end_detected_s
    free_decay_info = motion_info
    command_end_time_s = move_end_detected_s
    ...
    mask_bias = theta_valid & np.isfinite(theta_center_deg) & np.isfinite(pivot_x_corr)
    if np.sum(mask_bias) >= 10:
        coef = np.polyfit(pivot_x_corr[mask_bias], theta_center_deg[mask_bias], 2)
        theta_bias_x_deg = np.polyval(coef, pivot_x_corr)
    else:
    ...
    if is_stationary_noise_run:
        ...
        save_stationary_noise_profile(noise_profile_out_path, stationary_profile_out)
        print("\nSaved stationary noise profile:")
        print(f"  {os.path.abspath(noise_profile_out_path)}")
    ...
    ideal_x_in, ideal_info = generate_idealized_position_trace(time_s, pivot_x_in_report_smooth,
                                                               move_start_detected_s, move_end_detected_s,
                                                               command_speed_in_s=command_speed_in_s)
    delta_x_in, placement_info = compute_placement_metrics(time_s, pivot_x_in_report_smooth, ideal_x_in,
                                                           move_end_detected_s, goal_in=placement_goal_in,
    ...
    move_start_s=move_start_detected_s,
    move_end_s=move_end_detected_s
)
    ...
    if no_motion_detected:
        ...
        t_settle_band_s_measured, settled_observed, settle_censor_lb_s = measured_deadband_settling_with_dwell(
            time_s=time_s, theta_deg=theta_zeroed_deg, start_time_s=move_end_detected_s,
            band_deg=theta_band_deg, dwell_s=settling_deadband_dwell_s)
        t_settle_after_motion_end_s = (t_settle_band_s_measured - move_end_detected_s
                                      if settled_observed and np.isfinite(t_settle_band_s_measured) and np.isfinite(move_end_detected_s)
                                      else np.nan)
        t_settle_envelope_s, envelope_settled_observed, envelope_censor_lb_s, envelope_info = envelope_settling_time_from_peaks(
        time_s=time_s,
        theta_deg=theta_zeroed_deg,
        start_time_s=move_end_detected_s,
    ...
    t_settle_envelope_after_motion_end_s = (
        t_settle_envelope_s - move_end_detected_s
        if envelope_settled_observed and np.isfinite(t_settle_envelope_s) and np.isfinite(move_end_detected_s)
        else np.nan
    )
    ...
    print(f"  move travel [in]                 : {velocity_info.get('move_travel_in')}")
    print(f"  idealized target [in]            : {ideal_info.get('ideal_target_in')}")
    ...
    plt.axhline(theta_band_deg, color="k", linestyle="--", linewidth=1, label=f"+/-{theta_band_deg:g} deg deadband")
